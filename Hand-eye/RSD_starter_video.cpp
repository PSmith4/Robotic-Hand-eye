...
namespace {
    ...
    */
	...
	{	
		...
		findContours( objectMat, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point(0, 0) );
	...
	void  Robot_calabrate(RobotShell Robot,VideoCapture& capture, Mat background)
	{
		...
		for(int count=0; count<110; count++)
		{
			...
			Object_Detection(input,background, vector<HoldingBox>, grippers);
	...
	void RoboticMotion(Point2f pinPoint)
	{
		Robot.pickPin();
		Robot.moveToPosZero();
        ...
        for (;;) 
		{
            ...
            switch (key) 
			{
				...
				case 'M':
					try
					{
						RoboticMotion(meanMeasurment(pinPoints));
					}
