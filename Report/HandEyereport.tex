...
More specifically, this project has been completed for an industry client, AME System Pty. Ltd., for automated cavity plug insertion. These plugs are 3.5mm wide and are placed in cavities between 5 and 9mm apart, as shown in \cref{fig:IMG_20160908_142218}. Due to this small size, a high degree of accuracy and repeatability is required. 
\begin{figure}[h]
\centering
\includegraphics[width=0.5\linewidth]{IMG_20160908_142218}
\caption{Image of plug and socket}
...
\note{Something about looking at exact pin location, determining if pin is present.} 
...
\end{matrix}$
\note{add discussion of this algorithm, maybe even diagram of arm for how this was derived}\lipsum
\section{Matlab Simulations}
\note{matlab process here}\lipsum
\section{Open ABB}
...
\node [decision, below of=image21, node distance=3cm] (decide2) {Is a pin available to be picked?};
\node [block, below of=decide2, node distance=3.5cm] (pinget1) {Move to pin supply};
...
\node [end, right of=decide2, node distance=12cm] (Finish) {Finish} ;
...
\path [line] (image24) |- (decide2.north);
\path [line] (decide2) -- node [below]{no} (Finish);
\path [line] (decide2) -- node [left]{yes} (pinget1);
...
\path [line] (pinset6.south) |- ++(-18,-0.1) |- (start);
\end{tikzpicture}
