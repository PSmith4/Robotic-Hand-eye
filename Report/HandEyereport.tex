...
For each rectangle that was within the predicted size of the holding box or gripper, a classification and orientation was attempted as if the object were a holding box. If this process failed, the image was oriented as a gripper. If this failed, the object was deemed neither and discarded. From the example process, at the stage in \cref{fig:out3}, three objects remain; the holding box, the gripper and another part of the gripper of no interest.  Respectively, these objects would be assigned as a holding box; fail at being set as a holding box, and be assigned as a gripper, and fail at both assignings.\\
This process is shown in diagram in \cref{fig:Throwlinek}.
\begin{figure}
\centering
\includegraphics[width=0.25\linewidth]{Throwlinek}
...
\begin{figure}[h]
\centering
\includegraphics[width=1.2\linewidth]{aaah}
\caption{}
\label{fig:aaah}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=1\linewidth]{measumrnetflux}
\caption{}
...
\section{Pin supply identification}
Something about looking at exact pin location, determining if pin is present. 
...
\begin{table}
	...
	\begin{tabular}{| l | c c c c |}
		\hline
		Links & $\theta$ & d & a & $\alpha$ \\ [0.5ex]
		\hline
		1 		& $\theta$ & 0 	& 0 	& 0 		\\
		2 		& $\theta$2 	& 0 	& 0.07 	& $\pi$/2 	\\
		3 		& $\theta$3 	& 0 	& 0.36 	& 0 		\\
		4 		& $\theta$4 	& 0.38 	& 0 	& $\pi$/2 	\\
		5 		& $\theta$5 	& 0 	& 0 	& -$\pi$/2 	\\
		6 		& $\theta$6 	& 0.065 & 0 	& $\pi$/2 	\\ [1ex]
		\hline
	\end{tabular}
\end{table}
...
With the system's ability to know where to move to, and the systems ability to move there discussed, the operation flow chart can now be preseented. This is the standard operation for the system to place pins in required pinholes until the supply of pins is expended, or all pinholes are filled. 
...
\node [block] (start) {Move to postions 'Zero'};
\node [cloud, below of=start, node distance=2cm] (image11) {Take image};
...
\node [cloud, below of=decide3, node distance=2.5cm] (image13) {Measure pin distance};
\node [decision, below of=image13, node distance=3cm] (decide) {Enough measurements made?};
\node [cloud, below of=decide, node distance=3cm] (image21) {Average measrument};
...
\node [cloud, right of=image23, node distance=3cm] (image24) {detect pin in supply};
\node [decision, below of=image21, node distance=3cm] (decide2) {Is a pin ready to be picked?};
\node [block, below of=decide2, node distance=3cm] (pinget1) {Move to pin supply};
\node [block, right of=pinget1, node distance=3cm] (pinget2) {Grasp pin};
\node [block, below of=pinget1, node distance=2.5cm] (pinset1) {Move to pin hole};
\node [block, right of=pinset1, node distance=3cm] (pinset2) {lower};
\node [block, right of=pinset2, node distance=3cm] (pinset3) {release pin};
\node [end, right of=decide2, node distance=12cm] (stop) {Finish} ;
