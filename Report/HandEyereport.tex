...
After adapting the system to use a HD phone-camera, errors in the calculations of robot-frame pin positions were observed, see \cref{fig:error2}.
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{error2}
	\caption{Displays the magnitude (x10) and direction of the error observed for calculated robot-frame positions.}
	\label{fig:error2}
\end{figure}
The green dots are the expected robot-frame locations Xn, and the red lines are the error vectors Err, the vector from the actual location Xn, to the calculated estimate positions Mn, with magnitude scaled by 10 for observation purposes. \begin{equation}10(Err) = 10(Xn-Mn)\end{equation}
...
\section{Pin placement operation}
...
\end{subfigure} \ \ \
	\begin{subfigure}{0.4\linewidth}
		...
		\subcaption{Drive pin into hole}
	\end{subfigure} \ \ \
		\begin{subfigure}{0.4\linewidth}
		...
		\end{subfigure} \ \ \
			\begin{subfigure}{0.4\linewidth}
				...
				\subcaption{Drive pin into socket}
			\end{subfigure}
